...
import { bubbledns, classdata } from './main.js';
...
class tcpcommuicationclass {
    ...
    async create_connection() {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            if (classdata.db.routinedata.this_server.masternode == 1) //Masternode creates Server
            {
                var config = {
                ...
                that.server.em.on('command', async function (message) {
                    that.message_handler_server(message)
                 });
                ...
                resolve(`Communication-TCP-Server was started successfully and is listening on Port: ${that.config.port}/tcp`)
            }
            else //SlaveServer create Connection to all Master-Servers
            {
                ...
                var answer = ""
                    ...
                    try
                    {
                        ...
                        tcp_client.em.on('command', async function (message) {
                            that.message_handler_client(message)
                        });
    ...
    async message_handler_client(message)
    {
    ...
    async message_handler_server(message)
    {
